...
class Template:
    ...
    @staticmethod
    def import_moudle(module, abbr=None, indention=0):
        if abbr is None:
            return "%simport %s" %(" "* indention, module)
        else:
            return "%simport %s as %s" %(" "* indention, module, abbr)
    @staticmethod
...
class Translator:
    ...
    def process_parallel_for(self, line, struct):
        ...
        if parallel_line is None:
            ...
            self.writer.write(
                Template.import_moudle(
                    module="pathos.multiprocessing.ProcessingPool",
                    abbr="ProcessingPool",
                    indention=parallel_struct['indention']
                )
            )
    ...
    def header(self):
        self.writer.write(
            Template.import_moudle(
                module="pymp.helpers.join_dict",
                abbr="join_dict"
            )
        )
        self.writer.write(
            Template.import_moudle(
                module="pymp.helpers.join_shared",
                abbr="join_shared"
            )
        )
